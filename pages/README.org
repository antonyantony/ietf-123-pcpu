...
With 5000 source-port flows, ~32K packets traverse the slow path during the offload ramp-up period. These slow packets setup the nft offload path. And subsequent packets will take fast path. See the nft output bellow.
