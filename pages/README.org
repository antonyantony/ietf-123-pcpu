#+TITLE: Linux Kernel Flowtable Bulk Forwarding and XFRM pCPU Forwarding — Testing Results
...
All tests here are using smaller ethernet frame sizes, i.e 128 bytes due limitation of the system.  This is to show the scaling properties and later to be tested with 1518 byte frames.
...
With 5000 source-port flows, ~32K packets traverse the slow path during the offload ramp-up period. These slow packets setup the nft offload path. And subsequent packets will take fast path. See the nft output below ([[Flow offload][nft ruleset]]).
...
The crash is still under investigation. This is a blocker to submit the xdptools patch. I slightly suspect Mellanox driver not handling CPUMAP.
