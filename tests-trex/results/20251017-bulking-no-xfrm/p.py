...
# Average by run
avg_df = (
    df.groupby('flows')[['rx_pps', 'rx_throughput_gibps']]
    .mean()
    .reset_index()
    .sort_values('flows')
)
...
print_df = avg_df[['flows', 'rx_pps_mpps', 'rx_throughput_gibps']]
...
ax1.plot(x_vals, y_gbps_vals, marker='o', color='tab:blue', label='Bulk RX Gbps', linewidth=3)
...
ax2.plot(x_vals, y_pps_vals, marker='s', linestyle='-', color='tab:red', label='Bulk RX PPS (Mpps)', linewidth=3)
