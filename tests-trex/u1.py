...
import argparse
from trex.stl.api import STLClient, STLStream, STLTXCont, STLPktBuilder, STLTXSingleBurst
...
def create_pkt(src_mac, dst_mac, src_ip, dst_ip, frame_size, src_port, dst_port):
...
def run_priming(args, st, n=1, delay_ms=100):
    ...
    try:
        ...
        for flow_id in range(st.flows):
            ...
            pkt = create_pkt(args.src_mac, args.dst_mac, args.src_ip,
...
def run_test_frame_sizes(args, pps):
    ...
    for frame_size in args.frame_sizes: #loop over --frame-sizes array
        ...
        st = SimpleNamespace(pps=pps, frame_size=frame_size, flows=1, run=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dst-ip", type=str, default="10.25.2.2")
    parser.add_argument("--src-ip", type=str, default="10.25.0.1")
    ...
    parser.add_argument('--no-priming', action='store_true',
                        help='Disable priming', default=True)
